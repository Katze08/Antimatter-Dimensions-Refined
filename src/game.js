...
export function becomeIntergalactic() {
  if (!player.antimatter.exponent >= 70) return;
  if (NormalChallenge(8).isRunning) return;
  player.antimatter = player.antimatter.subtract(1e70);
  // There's a potential migration edge case involving already-maxed autobuyers; this should give the achievement (should do nothing currently, set to specific achievement if the galactical line is created)
  Achievement(61).tryUnlock();
...
export function init() {
  ...
  console.log("🌌 Antimatter Dimensions Refined Alpha 🌌");
  if (DEV) {
    // eslint-disable-next-line no-console
