...
import { GalaxyChallenge } from "@/core/galaxy-challenges";

export default {
  name: "GalaxyChallengeStartModal",
  components: {
    ModalWrapperChoice
  },
  props: {
  ...
  computed: {
    challenge() {
      return GalaxyChallenge(this.id);
    },
    ...
    message() {
      return `You will Big Crunch (if possible) and start a new Infinity within the Challenge with all the
        ...
        ${format(GalaxyChallenge(this.id).goal)} Antimatter.
        You do not start with any Dimension Boosts or Galaxies, regardless of upgrades.`;
    },
    entranceLabel() {
      return `You are about to enter Galaxy Challenge ${this.id}`;
    },
    reward() {
    ...
    condition() {
      ...
      return `Inside this Galaxy Challenge, ${conditionOfChallenge}`;
    }
  },
  created() {
